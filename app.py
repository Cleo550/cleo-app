...
M_S = st.selectbox("Mes", MSS, index=datetime.now().month-1)
...
# --- DIBUJO DE FACTURA ---
if st.button("GENERAR FACTURA PROFESIONAL"):
    img = Image.new('RGB', (1000, 1200), color=(255, 255, 255))
    d = ImageDraw.Draw(img)
    
    # 1. Cabecera con estilo
    d.rectangle([0, 0, 1000, 150], fill=(240, 240, 240))
    d.text((50, 50), "🧹 FACTURA DE SERVICIOS", fill=(0,0,0))
    d.text((700, 50), f"Nº: {nf}\nFecha: 01/{M_I:02d}/2026", fill=(80,80,80))

    # 2. Emisor y Cliente
    d.text((50, 200), "EMISOR:", fill=(150, 150, 150))
    d.text((50, 230), MI_DATOS, fill=(0, 0, 0))
    d.text((550, 200), "CLIENTE:", fill=(150, 150, 150))
    d.text((550, 230), f"{C['n']}\nNIF: {C['f']}\n{C['d']}", fill=(0, 0, 0))

    # 3. Tabla (Alineada)
    y = 450
    d.rectangle([50, y, 950, y+40], fill=(230, 230, 230))
    d.text((60, y+10), "CONCEPTO", fill=(0,0,0))
    d.text((450, y+10), "FECHA", fill=(0,0,0))
    d.text((600, y+10), "H", fill=(0,0,0))
    ...
    # 4. Bloque Total
    y += 30
    d.rectangle([650, y, 950, y+60], fill=(255, 245, 245))
    total_f = h_tot * C['t']
    ...
    d.text((50, y_p+20), LEY, fill=(120, 120, 120))
    p_txt = f"PAGO: {IBAN}" if C_S != "Ania" else "PAGO: En Efectivo"
