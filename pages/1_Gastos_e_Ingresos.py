...
with tab_bbva:
    ...
    if mi in meses_pago_130:
        ...
        # El mes de pago del primer trimestre es el primer mes del trimestre siguiente
        # Clave donde Modelo 130 guarda el importe calculado
        primer_mes_trim = {1: 1, 2: 4, 3: 7, 4: 10}[t_anterior]
        importe_130 = float(get_dato_local(_datos, f"mod130_importe_t{t_anterior}_{anio_t}", 0.0))
        if importe_130 == 0.0:
            # Fallback: leer del apartado Mod.130 en Gastos e Ingresos
            importe_130 = float(get_dato_local(_datos, f"mod130_Mod._130_(trimestral)_{primer_mes_trim}_{anio_t}", 0.0))
