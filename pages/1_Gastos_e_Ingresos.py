...
with tab_bbva:
    ...
    if mi in meses_pago_130:
        ...
        if importe_130 == 0.0:
            # Si no está guardado aún, intentar leer el valor del Mod.130 en Gastos e Ingresos
            importe_130 = float(get_dato(f"mod130_Mod._130_(trimestral)_1_{anio_t}", 0.0))
        ya_pagado = get_dato(f"mod130_pagado_t{t_anterior}_{anio_t}", False)
        if importe_130 > 0:
